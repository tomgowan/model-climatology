...
for j in range(798):
        ...
        if quart_snotel[j,0] > 0: # Make sure its a station
            quart_snotel[j,2] = np.percentile(temp, 85)
            dec_snotel[j,2] = np.percentile(temp, 95)
...
for i in range(621):
    for j in range(1405):    
        ...
        if len(prism_temp) > 1: #Make sure entire array wasnt deleted
            prism_quart[i,j] = np.percentile(prism_temp, 85)
            prism_dec[i,j] = np.percentile(prism_temp, 95)
...
#%%
fig = plt.figure(figsize=(17,21.5))
...
levels = [0, 0.5,1,1.5,2,2.5,3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7,  8, 9,10, 11, 12, 13, 14.5, 16, 17.5, 19,21, 24, 30,36, 42, 50, 60, 70, 80, 100,140]
levels_ticks = [0,1.5, 3.5,  5,  6.5, 9, 12, 16, 21, 36,  60, 100]
...
plt.text(80000,100000,'(a) SNOTEL $\mathregular{85^{th}}$ Percentile',fontsize = 28, bbox = props)
...
plt.text(80000,100000,'(b) PRISM $\mathregular{85^{th}}$ Percentile',fontsize = 28, bbox = props)
...
plt.text(80000,100000,'(c) SNOTEL $\mathregular{95^{th}}$ Percentile',fontsize = 28, bbox = props)
...
plt.text(80000,100000,'(d) PRISM $\mathregular{95^{th}}$ Percentile',fontsize = 28, bbox = props)
...
plt.savefig("../../../public_html/snotel_prism_event_size_85_95.png")
