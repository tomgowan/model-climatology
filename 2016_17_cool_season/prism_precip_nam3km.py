...
for i in range(0,days): 
    ...
    for hour_s in range(12,34,3):
        hour_f = hour_s + 3
        nam3km_file = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/nam3km/%08d" % Date[i] + "_%s_" % hour_s + "%s" % hour_f + "_nam3km_prism_interp.nc"
        nam3km_file2 = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/nam3km/%08d00" % Date[i] + "_%s_" % hour_s + "%s" % hour_f + "_nam3km_prism_interp.nc"
        if os.path.exists(nam3km_file):
            x = x + 1
        if os.path.exists(nam3km_file2):
            x = x + 1
    ...
    if x == 8 and z == 1:
        ...
        for hour_s in range(12,34,3):
            ...
            if os.path.exists(nam3km_file):
                ...
                grbs = pygrib.open(nam3km_file)
                msg = grbs[1]
                precip_vals = msg.values
                lats_hrrr,lons_hrrr = msg.latlons()
                precip_vals = precip_vals*0.0393689*25.4
                totalprecip = totalprecip+precip_vals
                
            if os.path.exists(nam3km_file2):

                grbs = pygrib.open(nam3km_file2)
                msg = grbs[1]
                precip_vals = msg.values
                lats_hrrr,lons_hrrr = msg.latlons()
                precip_vals = precip_vals*0.0393689*25.4
...
#%%
np.savetxt('nam3km_dailymean.txt', totalprecip)
np.savetxt('prism_nam3km_dailymean.txt', precip_tot)
#%%
...
plt.savefig("../../../public_html/nam3km_prism_climo_2016_17.pdf")
plt.show()
