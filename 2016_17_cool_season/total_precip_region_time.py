...
for i in range(0,2):
    ...
    if i == 1:
        ax1.text(3, 770, '(b) Interior Ranges', fontsize = 25, bbox = props)
    ...
    if i == 0:
        ax1.text(3, 1810, '(a) Pacific Ranges', fontsize = 25, bbox = props)
    ...
    black = mlines.Line2D([], [],linewidth = 2, marker = "o", markeredgecolor = 'none', color='k',
                           label='SNOTEL')
    

    h = max(precip_final[0,:,3])  
    l = max(precip_final[1,:,3])        
    
    ax1.fill_between(x, precip_final[i,:,6], 0, facecolor = 'saddlebrown',alpha=0.25)
    if i == 0:
        ax1.fill_between(x, precip_final[i,:,6], h, facecolor = 'darkgreen',alpha=0.25)
    ...
    plt.ylabel('Total Precipitation (mm)', fontsize = 20, labelpad = 13)

    if i == 1:
        
        plt.xlabel('Day of Cool-Season', fontsize = 20, labelpad = 13)

plt.legend(handles=[ blue, red, cyan, green, gold, magenta, black], loc='upper center', bbox_to_anchor=(.160, 2), 
           ncol=1,fontsize = 16)
plt.tight_layout()
subplots_adjust(left=None, bottom=.08, right=None, top=.95, wspace=.07, hspace=.11) 

plt.savefig("../../../public_html/mean_accum_precip_region_2016_17.pdf")
plt.show()
