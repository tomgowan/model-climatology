...
levels = [1.5,1.75, 2, 2.25, 2.5, 2.75, 3, 3.5, 4, 4.5, 5,5.5, 6, 6.5, 7, 7.5, 8, 9,11, 13,  16, 22, 30,50]
levels_ticks = [1.5, 2,  2.5,  3,  4,  5,  6,   7, 8, 11, 16, 30]
...
csAVG = map.contourf(x,y,prism_med, levels,  cmap = cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N))
map.drawcoastlines(linewidth = .5)
map.drawstates()
map.drawcountries()
plt.title('PRISM', fontsize = 28)
...
xi, yi = map(med_snotel[:,1], med_snotel[:,0])
csAVG = map.scatter(xi,yi, c = med_snotel[:,2], cmap=cmap, marker='o', norm=matplotlib.colors.BoundaryNorm(levels,cmap.N), s = 120)#,norm=matplotlib.colors.LogNorm() )  
...
cbar.ax.set_xlabel('Median Event Size (mm)', fontsize = 24, labelpad = 15)

plt.tight_layout()
plt.savefig("../../../public_html/snotel_prism_med_event_size.png")
