...
totalprecip_arw = zeros((621,1405))
totalprecip_nmb = zeros((621,1405))
...
for i in range(0,days): 
    ...
    for core in ['arw', 'nmb']:
        for mem in ['ctl']:
            for hour in range(12,34,3):
                sref_file = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/sref/%08d03" % Date[i] + "%s" % core + '_%s' % mem + 'F%02d' % hour + "_sref_prism_interp.nc"
                if os.path.exists(sref_file):
                    x = x + 1
    ...
    if x == 16 and z == 1:
        ...
        for core in ['arw', 'nmb']:
            for mem in ['ctl']:
                for hour in range(9,34,3):
                    sref_file = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/sref/%08d03" % Date[i] + "%s" % core + '_%s' % mem + 'F%02d' % hour + "_sref_prism_interp.nc"
                
                    if core == 'arw':
                        grbs_arw = pygrib.open(sref_file)
                        msg = grbs_arw[2]
                        precip_vals_arw = msg.values
                        precip_vals_arw = precip_vals_arw*0.0393689*25.4
                        totalprecip_arw = totalprecip_arw+precip_vals_arw
                    
                    if core == 'nmb':
                        grbs_nmb = pygrib.open(sref_file)
                        msg = grbs_nmb[1]
                        precip_vals_nmb = msg.values
                        precip_vals_nmb = precip_vals_nmb*0.0393689*25.4
                        totalprecip_nmb = totalprecip_nmb+precip_vals_nmb
...
totalprecip_arw = totalprecip_arw/y
totalprecip_nmb = totalprecip_nmb/y
...
'''
np.savetxt('sref_arw_ctl_dailymean.txt', totalprecip_arw)
np.savetxt('prism_sref_arw_dailymean.txt', precip_tot)


np.savetxt('sref_nmb_ctl_dailymean.txt', totalprecip_nmb)
np.savetxt('prism_sref_nmb_dailymean.txt', precip_tot)
...
csAVG = map.contourf(x,y,totalprecip_arw, levels, cmap = cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N))
...
x, y = map(lons_prism, lats_prism)
precip_tot = maskoceans(lons_prism, lats_prism, precip_tot)
csAVG = map.contourf(x,y,totalprecip_nmb, levels, cmap = cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N))
map.drawcoastlines(linewidth = .5)
map.drawstates()
map.drawcountries()
cbar = map.colorbar(csAVG, location='bottom', pad="5%")
...
plt.savefig("../../../public_html/sref_prism_climo_2016_17.pdf")
plt.show()
