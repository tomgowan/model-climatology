...
for i in range(0,days): 
    ...
    for hour in ['03', '15']:
            for run in ['09', '21']:
                hrrr_file = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/hrrr/%08d" % Date[i] + "%s" % run + "%s" % hour + "hrrr_prism_interp.grib2"
    ...
    print x
    if x == 4 and z == 1:
        totalprecip15 = zeros((621,1405)) 
        totalprecip3 = zeros((621,1405))
        y = y + 1
        #############  GFS   ############################

        for run in ['09', '21']:
    
                HRRR_File = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/hrrr/%08d" % Date[i] + "%s" % run + "15hrrr_prism_interp.grib2"
                HRRR_File3 = "/uufs/chpc.utah.edu/common/home/steenburgh-group5/tom/model_prism_interp/2016_17_cool_season/hrrr/%08d" % Date[i] + "%s" % run + "03hrrr_prism_interp.grib2"
                ...
                msg3 = grbs3[1]
                precip_vals3 = msg3.values
                precip_vals3 = precip_vals3*0.0393689*25.4
                totalprecip3 = totalprecip3 + precip_vals3
                        

                grbs = pygrib.open(HRRR_File)
                msg = grbs[1]
                precip_vals = msg.values
                lats_hrrr,lons_hrrr = msg.latlons()
                precip_vals = precip_vals*0.0393689*25.4
                totalprecip15 = totalprecip15 + precip_vals
                
        totalprecip = totalprecip + (totalprecip15-totalprecip3)
...
np.savetxt('hrrr_dailymean.txt', totalprecip)
...
csAVG = map.contourf(x,y,totalprecip, levels, cmap = cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N))
...
avg1 = totalprecip[17:453, 0:540]/precip_tot[17:453, 0:540]
...
x, y = map(lons_prism, lats_prism)
csAVG = map.contourf(x,y,totalprecip/precip_tot, levels,cmap=cmap, norm=matplotlib.colors.BoundaryNorm(levels,cmap.N), vmin = 0.1, vmax = 5)
map.drawcoastlines(linewidth = .5)
map.drawstates()
map.drawcountries()
cbar.ax.tick_params(labelsize=12)
...
plt.savefig("../../../public_html/hrrrr_prism_climo_2016_17.png")
plt.show()
